...
arg[3] <- "C:/Users/croemer01/Documents/Donnees vigie-chiro/Cartes/VC_90_DI" # repertory with outputs from Predict_act
arg[4] <- "C:/Users/croemer01/Documents/Post-Doc/Vigie-Chiro et Indicateurs_ecologiques/Stages/Sarah Chayrigues/Cartes/Figures VC_90_DI" #repertory for png
...
for (i in 1:length(names(table(file_bind$Species)))) { # For each species
  ...
  for (j in 1:length(names(table(dataa$Month)))){
    png(filename=paste0(arg[4], "/", Sp,"_",names(table(dataa$Month))[j],".png"), width = 3000, height = 2500, res=300)
    
    dataa_Month=subset(dataa, dataa$Month==names(table(dataa$Month))[j])
  ...
  dataa_april=subset(dataa, dataa$Month=="4")
  dataa_july=subset(dataa, dataa$Month=="7")
  dataa_sept=subset(dataa, dataa$Month=="9")
